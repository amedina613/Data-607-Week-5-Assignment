...
```{r}
raw_data <- raw_data %>%
  pivot_longer(cols = c(Los.Angeles, Phoenix, San.Diego, San.Francisco, Seattle), names_to = "Destination", values_to = "Time")
...
```{r}
raw_data<- raw_data %>%
  pivot_wider(names_from = Arrival, values_from = Time)
...
AM west has much greater mean and median delay times than Alaska airlines as well as higher maximum and minimum delay times. 