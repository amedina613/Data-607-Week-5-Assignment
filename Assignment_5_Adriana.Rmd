...
```{r}
raw_data <- raw_data %>%
  pivot_longer(cols = c(Los.Angeles, Phoenix, San.Diego, San.Francisco, Seattle), names_to = "Destination", values_to = "num_of_flights")
...
```{r}
raw_data<- raw_data %>%
  pivot_wider(names_from = Arrival, values_from = num_of_flights)
...
AM West has a total of 787 delayed flights. Alaska has a total of 501 delayed flights. 
...
airline_ot <- ggplot(raw_data, aes(x = Airline, y = `on time`, fill = Airline)) +
  ...
  labs(title = "Comparison of On Time Flights for Different Airlines",
       x = "Airline", y = "Number of On-time Flights") +
  theme_minimal()
